...
test.describe("Drag and Drop Functionality", () => {
  ...
  test("should display dimensions in sidebar and they should be draggable", async ({
    page,
  }) => {
    ...
    await expect(
      sidebar.getByRole("button", { name: /Dimensions/i }),
    ).toBeVisible();
    ...
    await expect(nameInSidebar.locator("xpath=../../..")).toHaveClass(
      /cursor-pointer/,
    );
  });

  test("should allow dragging dimensions to Group By zone", async ({
    page,
  }) => {
    ...
    // helper that mimics pointer drag using bounding boxes
    async function drag(sourceLocator, targetLocator) {
    ...
    await drag(dimensionElement, groupByZone);
  ...
  test("should display dimensions and metrics in sidebar tabs", async ({
    page,
  }) => {
    const sidebar = page.locator("div.bg-gray-50.border-r");
    ...
    await expect(sidebar.getByText("age", { exact: true })).toBeVisible({
      timeout: 10000,
    });
    await expect(sidebar.getByText("salary", { exact: true })).toBeVisible({
      timeout: 10000,
    });
  ...
  test("should have proper visual feedback for drag operations", async ({
    page,
  }) => {
    const sidebar = page.locator("div.bg-gray-50.border-r");
